...
unifyTerms f@(Function _ _ tms ) v@(Variable _ ) =
     | v `occursAt` tms = Nothing
     | otherwise        = Just [(f,v)]
--occurs check needed
unifyTerms v@(Variable _ ) f@(Function _ _ _ )  = Just [(f,v)]
...
unifyTerms (Function name1 arity1 tms1) (Function name2 arity2 tms2)
   ...
   | otherwise                   = concat (zipWith unifyTerms tms1 tms2)

------------------------------------------------------------
occursAt :: Term -> Term -> Bool
v@(Variable str) `occursAt` (Funtion _ _ tms) = or (map (occursAt v) tms)
